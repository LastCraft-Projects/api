...
public final class JoinMessage {
    ...
    private JoinMessage(int id, String message, int priceGold, boolean shulker) {
        this.id = id;
    ...
    public String getMessage() {
        if (message == null) {
            return Language.getDefault().getMessage("JOIN_PLAYER_LO_LOBBY");
        }
    ...
    public boolean can() {
        return shulker || priceGold > 0;
    }

    private static final TIntObjectMap<JoinMessage> MESSAGES = TCollections.synchronizedMap(new TIntObjectHashMap<>());

    public static final JoinMessage DEFAULT_MESSAGE = new JoinMessage(1, null, 0, true);

    public static List<JoinMessage> getMessages() {
        return MESSAGES.valueCollection().stream()
    ...
    private static void addMessage(int id, String message, int priceGold, boolean shulker) {
        MESSAGES.put(id, new JoinMessage(id, message, (priceGold > 0 ? priceGold : 0), shulker));
    }
