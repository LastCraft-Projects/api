...
import net.lastcraft.base.sql.api.StatementWrapper;
...
public class GlobalLoader {
    ...
    private final String GET_LICENSE_QUERY = "SELECT * FROM `premium_auth` WHERE `player_name` = ? LIMIT 1;";
    private final String UPD_LICENSE_QUERY = "UPDATE `premium_auth` SET `using_premium_auth` = ? WHERE `player_name` = ?";
    private final String INSERT_LICENSE_QUERY = "INSERT INTO `premium_auth` (`player_name`, `using_premium_auth`) VALUES (?, ?)";
     ...
     * map - настройки
     */
    public Pair<Pair<Language, Boolean>, Map<SettingsType, Boolean>> getSettings(int playerID) {
        if (playerID == -1) {
            return new Pair<>(new Pair<>(Language.getDefault(), false), new HashMap<>());
        }
        ...
        return MYSQL_DATABASE.executeQuery(GET_SETTINGS_QUERY, (rs) -> {
            ...
            Map<SettingsType, Boolean> settings = new HashMap<>();
    ...
    public TIntList getFriends(int playerID) {
        TIntArrayList friends = new TIntArrayList();
    ...
    public String getPrefix(int playerID, Group group) {
        ...
        if ((group != Group.SHULKER && group != Group.ADMIN) || prefix.toLowerCase().contains("test")) {
            return prefix;
        }
